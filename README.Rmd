...
An work-in-progress R package containing simple functions to help me train myself to 'read' a colour from its hex code. I'm colourblind (a deuteranope) so this might be a useful skill.
...
And if you're wondering what the colour actually is, you can either set `swatch = FALSE` in `dh_solve()`, or use `dh_swatch()`.

```{r ex-swatch, fig.alt="A square of colour with its three-digit hex colour code in text in the centre."}
